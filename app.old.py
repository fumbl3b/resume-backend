...
def generate_optimized_resume(resume_text: str, suggestions: str, model=DEFAULT_MODEL) -> str:
    messages = [
        {"role": "system", "content": "You are a Professional LaTeX resume formatter. Respond only with valid LaTeX code. Do not include any other text or explanations."},
        {"role": "user", "content": (
            ...
            "Return ONLY the LaTeX code with no additional text.  Do NOT include any backticks or non-LaTeX valid syntax. This response will be directly input into a\n\n"
            f"SUGGESTIONS:\n{suggestions}\n\n"
            f"RESUME:\n{resume_text}"
        )}
    ]
...
def api_generate_optimized_resume():
    ...
    # Step 2: Try PDF Conversion (Optional)
    try:
